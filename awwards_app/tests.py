...
class RateTest(TestCase):
  def setUp(self):
    self.user = User.objects.create(id=1, username='FloiceNyota')
    self.project = Project.objects.create(id=1, title='Random project', description='Random project description',posted_on='2021,6,19',project_image='https://cloudinary url', repo_link='http://github.com',live_link='http://heroku.com',user=self.user,technologies_used='Python')
...
class TestProfile(TestCase):
  def setUp(self):
    self.user = User(id=1, username='FloiceNyota', password='Moringa')
    self.user.save()
